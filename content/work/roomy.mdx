...
<Summary
  readTime="3 min"
  problem="Twenty-one people sharing a flat, with rent chased through WhatsApp, cash handoffs and no receipts, and one washing machine nobody could book."
  approach="Designed and shipped a shared system of record: a rent ledger that resolves every balance, and a live laundry scheduler that prevents conflicts in the data layer."
  constraint="Most trust problems in shared living are visibility problems, so the real design work was scoping who sees what, not adding features."
/>
