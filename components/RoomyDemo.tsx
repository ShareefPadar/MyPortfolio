...
const RoomyDemo = ({ animate = true, scale = 1 }: { animate?: boolean, scale?: number }) => {
  ...
  return (
    <div className="w-full max-w-sm mx-auto bg-slate-50 rounded-3xl shadow-2xl border-8 border-white overflow-hidden font-sans text-slate-900">
      {/* HEADER SECTION - Based on Screenshot */}
      <div className="bg-slate-900 p-6 text-white pb-10 rounded-b-3xl relative overflow-hidden">
      ...
      <div className="px-6 -mt-6 relative z-20">
        <div className="bg-white p-1.5 rounded-2xl shadow-lg flex gap-1 border border-slate-100">
          <button 
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-xs font-bold transition-all ${activeTab === "ledger" ? "bg-slate-900 text-white" : "text-slate-400"}`}
          >
            <LayoutDashboard className="w-4 h-4" /> Rent Ledger
          </button>
          <button 
            onClick={() => setActiveTab("approvals")}
            className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-xs font-bold transition-all ${activeTab === "approvals" ? "bg-slate-900 text-white" : "text-slate-400"}`}
          >
      ...
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs font-bold bg-slate-200 text-slate-500 px-2 py-1 rounded-md uppercase tracking-wider">March 2026</span>
        </div>
        ...
        <div className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100 flex flex-col gap-4">
          ...
          {isAdmin && !isPaid && (
            <motion.button 
              ...
              className="w-full bg-slate-900 text-white py-3.5 rounded-2xl font-bold text-xs shadow-md shadow-slate-200"
            >
              Confirm Payment
            </motion.button>
          )}
